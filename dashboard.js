...
async function render() {
  ...
  const data = await browser.storage.local.get(['day:'+selected,'paused','goals:'+selected,'recommendations:'+selected,'purposes:'+selected]);
  if (date.value !== selected) return;
  rows = data['day:'+selected] || []; recommendations = data['recommendations:'+selected] || []; paused = !!data.paused;
  groupedRows = Ledger.group(rows, data['purposes:'+selected] || {});
  ...
  const short = groupedRows.filter(r=>{const seconds=r.seconds.foreground+r.seconds.backgroundAudio+r.seconds.backgroundSilent;return r.videoId && seconds>0 && seconds<=180;}).length;
  $('recommendation-summary').textContent = `${reveals} deliberate reveals · ${visible} detected on screen · ${short} videos played for ≤3 min today (includes videos in progress)`;
