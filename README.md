...
Choose **Retrowave**, **Light green**, or **Dark green** under Advanced settings → Dashboard theme, which applies and saves immediately. Other settings still use Save settings. Dark green is the default. Saved theme choices are preserved. Light green uses a light background, green accents, and a compact header. Theme selection is saved per browser profile.
