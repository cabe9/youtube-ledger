...
(()=>{
  ...
  function updateCard(card){
    ...
    const time=card.querySelector('time');if(time)time.textContent=Ledger.relativeTime(Number(card.dataset.publishedAt));
    updateDuration(card);
    updateViews(card);
  }
  function updateViews(card){
    ...
    label.textContent=(count===undefined?'—':format.format(count))+' views'+(kind==='rate'?' · '+(rate===undefined?'—':rate>0&&rate<1?'<1':format.format(rate))+' views/hour':'');
    label.title=Ledger.validVideoViews(entry.views)?entry.views.count.toLocaleString()+' views · checked '+new Date(entry.views.checkedAt).toLocaleString()+(kind==='rate'?'. Average from upload until that check; not current viewing activity.':''):'View count unavailable. Unknown values sort last.';
  }
  ...
  function renderMembers(root,group,sameView){
    ...
    for(const channel of data.channels){
      ...
      if(channel.error){if(!error){error=el('small');row.append(error);}error.textContent=channel.error;error.title=channel.retryAt?'Next automatic retry after '+new Date(channel.retryAt).toLocaleTimeString():'';}else error?.remove();
      rows.push(row);
    }
