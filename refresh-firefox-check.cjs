...
(async()=>{
 ...
 fs.writeFileSync(path.join(extension,'test-paced.js'),`(async()=>{
  ...
  try{
   ...
   verify(calls.length===8&&missing.pausedUntil>Date.now()&&missing.entries.length===3,'Three initial 404s must stop the run and retain cached videos');
   ...
   verify(paused.pauseScope==='automatic'&&paused.pauseReason==='feed-not-found'&&paused.pauseMessage.includes('Three channel upload feeds returned HTTP 404'),'Feed pause must explain its scope');
 ...
 try{
  ...
  socket.onmessage=event=>{const message=JSON.parse(event.data),item=pending.get(message.id);if(!item)return;pending.delete(message.id);clearTimeout(item.timer);message.type==='error'?item.reject(Error(message.error+': '+message.message)):item.resolve(message.result);};
  const send=(method,params={})=>new Promise((resolve,reject)=>{const id=++sequence,timer=setTimeout(()=>reject(Error(method+' timeout')),30000);pending.set(id,{resolve,reject,timer});socket.send(JSON.stringify({id,method,params}));});
  const session=await send('session.new',{capabilities:{}});await send('webExtension.install',{extensionData:{type:'path',path:extension}});
  let page;for(let i=0;i<100&&!page;i++){page=(await send('browsingContext.getTree',{})).contexts.find(c=>c.url.endsWith('/test-result.html'));if(!page)await sleep(500);}
  ...
  console.log('PASS: Firefox '+session.capabilities.browserVersion+' early 404 run detection, shared pacing, cached reads, manual additions during feed pauses and server cooldowns; '+report.requests+' synthetic requests.');
