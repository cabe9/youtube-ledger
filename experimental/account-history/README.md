...
Outputs: `dist/experimental/chrome`, `dist/experimental/firefox`, and `youtube-ledger-{browser}-experimental-0.16.18.zip`. The manifest name is **YouTube Ledger Experimental**. The default `python3 build.py` instead creates the store packages in `dist/chrome` and `dist/firefox`; `python3 package-check.py` verifies their exclusion boundary and stale-file cleanup. Generated packages have not been published.
